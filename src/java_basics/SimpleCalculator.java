...
public class SimpleCalculator {
    ...
    public static float[] readOperands(Scanner scanner){
        ...
        for(int i=0; i<operands.length;i++){
            while(true) {
                try {
                    System.out.println(STR."Enter operand (\{i+1})");
                    operands[i] = scanner.nextFloat();
                    break;
                } catch (InputMismatchException e) {
                    System.out.println(STR."Error: Invalid input. Please enter a valid number");
                    scanner.nextLine();
                }
            }
        }
    ...
    public static void main(String[] args){
        ...
        while(true){
            ...
            if(choice < 1 || choice > 6){
                System.out.println("Enter a number value between 1 and 6");
                scanner.nextLine();
                continue;
            }
            ...
            switch(choice){
                case 1:
                    ...
                    System.out.println(STR."The sum of \{num1} and \{num2} = \{result}");
                    break;
                case 2:
                    result = subtract(num1, num2);
                    System.out.println(STR."The difference between \{num1}  and \{num2} = \{result}");
                    break;
                case 3:
                    result = multiply(num1, num2);
                    System.out.println(STR."The product of \{num1} and \{num2} = \{result}");
                    break;
                case 4:
                    result = divide(num1, num2);
                    if(!Float.isNaN(result)){
                        System.out.println(STR."The quotient of \{num1} and \{num2} = \{result}");
                    }
                    break;
                case 5:
                    result = modulus(num1, num2);
                    if(!Float.isNaN(result)){
                        System.out.println(STR."The modulus of \{num1} and \{num2} = \{result}");
                    }
                    break;
                default:
